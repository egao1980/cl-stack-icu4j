...
(progn
  ...
  (deftest uchar-properties
    ...
    (ok (search "control" (string-downcase (or (char-name 0 :choice :extended) ""))))
    (ok (= (simple-to-title #x01C5) #x01C5))
    (ok (= (simple-to-upper #x01C5) #x01C4))
    (ok (member :latin (script-extensions (char-code #\A))))
  ...
  (deftest number-date-list
    ...
    (let ((ts (format-time 0 :locale "en-US" :style :short)))
      (ok (plusp (length ts)))
      (ok (or (find #\: ts) (search "AM" ts) (search "PM" ts)
              (search "am" ts) (search "pm" ts))))
    (ok (plusp (length (format-datetime 0 :locale "en-US"))))
    (let* ((ds (format-date 0 :locale "en-US" :style :short))
           (ut (parse-date ds :locale "en-US" :style :short)))
      (ok (numberp ut)))
    (let ((s (format-relative-time -1 :day :locale "en" :numeric t :style :short)))
      (ok (plusp (length s))))
    (let ((and-s (format-list '("a" "b" "c") :locale "en" :type :and :width :narrow))
