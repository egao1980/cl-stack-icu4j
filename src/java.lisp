...
(progn
  ...
  (defun %uproperty (property)
    "Protocol/Lisp keyword → UProperty int field (HEX_DIGIT, ID_START, …)."
    (let* ((known
             '((:white-space . "WHITE_SPACE")
               ...
               (:posix-blank . "POSIX_BLANK")
               (:bidi-mirrored . "BIDI_MIRRORED")))
           (name (or (cdr (assoc property known :test #'eq))
                     (substitute #\_ #\- (symbol-name property) :test #'char-equal))))
      (%jfield "com.ibm.icu.lang.UProperty" (string-upcase name))))
